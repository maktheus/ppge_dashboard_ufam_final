...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isCollapsed, onToggle, userRole, onUserChange, currentUser, onLoginRequest, onLogoutRequest }) => {
  const allNavItems = [
          ...
          {navItems.map((item) => (
            <li key={item.id}>
              <button
                ...
                className={`w-full flex items-center py-3 rounded-lg transition-colors duration-200 ${isCollapsed ? 'px-4 justify-center' : 'px-4'} ${activeView === item.id
        ...
        {userRole === 'Visualizador' ? (
          ...
          <div className="flex flex-col space-y-2">
            <div className="text-xs text-gray-500 dark:text-gray-400">
              Logado como: <span className="font-semibold text-gray-700 dark:text-gray-200">{currentUser.name}</span>
            </div>
            <button
              onClick={onLogoutRequest}
              className="w-full flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
            >
              Sair (Modo Visualizador)
            </button>
          </div>
        )}
      </div>
