...
const DataTable: React.FC<DataTableProps> = ({ data, onUpdate, onDelete, userRole }) => {
  const [sortKey, setSortKey] = useState<SortKey>('anoIngresso');
  ...
  const sortedData = useMemo(() => {
    return [...data].sort((a, b) => {
      const isDateKey = ['anoIngresso', 'anoDefesa'].includes(sortKey);
      ...
      // Secondary sort: Nome (Alphabetical)
      if (result === 0 && sortKey !== 'nome') {
        return compareValues(a.nome, b.nome, 'asc', { ignoreAccents: true });
      }
