...
const Dashboard: React.FC<DashboardProps> = ({ graduates, docentes, turmas, onUpdate, onDelete, projetos, onUpdateProjeto, onDeleteProjeto, userRole }) => {
  ...
  useEffect(() => {
    ...
    if (defenseYears.length > 0) {
      const minYear = Math.min(...defenseYears);
      const maxYear = Math.max(...defenseYears);
  ...
  const getTabClassName = (tabName: 'egressos' | 'projetos') => {
    ...
    return `whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm cursor-pointer ${isActive
